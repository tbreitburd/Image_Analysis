...
In this exercise, 3 images were used to test different segmetentation methods. For each image, a specific segmentation objective was set, and 3 differemt main algorithms were used to segment the images, alomg with some additional methods.

\subsection{Image 1: \texttt{CT.png}}

The first image, \texttt{CT.png}, is a CT scan of a human torso. The objective of this segmentation is to segment the lungs from the rest of the image, including any tissue or nodule inside it. This segmentation was done using a region-growing algorithm. EXPLAIN. The first part of the segmentation was to divide the image into regions, from which it was then possible to set seeds for the region-growing algorithm inside each lung. This was done using thresholding and closing. Thresholding is a simple method to segment an image by setting a threshold value, and then creating a binary image where all pixels above the threshold are set to 1 (0 otherwise). Setting the threshold value is done by selecting the value which maximises the between-class variance of the resulting binary image. CLosing is a morphological operation that is used to fill in small holes (locations with value 0) in the binary image. The first step is to grow then erode????

Once this was done, regions were identified. This is done by taking each continuous area of a single value in the binary image, and assigning it a unique label. From there, the lungs were identified as the 2 smallest regions. The seeds for the region-growing algorithm were then set as one of the points in that region (centroids are less robust as they could be outside the actual region, if it is not convex). The region-growing algorithm then grows the region by adding neighbouring pixels that are within a certain threshold of the current region. This is done iteratively until the region stops growing. The entire process is shown in Figure \ref{fig:ct_segmentation}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{../Plots/CT_Segmentation.png}
...
Here, the task was to segment purple tulips from an image of a Dutch tulip field. The main challenge here was the presence of noise in the image rendering the
